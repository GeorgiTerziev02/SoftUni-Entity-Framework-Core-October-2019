namespace PetStore.Services.Implementations
{
    ...
    public class PetService : IPetService
    {
        ...
        public void BuyPet(Gender gender, DateTime dateOfBirth, decimal price, string description, int breedId, int categoryId)
        {
            if (price < 0)
            {
                throw new ArgumentException("Pet price cannot be less than zero!");
            }

            if (!this.breedService.Exists(breedId))
            {
                throw new ArgumentException("There is no such breed with the given id in the database!");
            }

            if (!this.categoryService.Exists(categoryId))
            {
                throw new ArgumentException("There is no such category with the given id in the database!");
            }
        ...
        public void SellPet(int petId, int userId)
        {
            ...
            {
                throw new ArgumentException("There is no such pet with the given id in the database!");
            }
